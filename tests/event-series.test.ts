...
beforeAll(async () => {
  ...
  await db.insert(events).values([
    { title: "Serie", kind: "rehearsal", date: isoTag(-14), seriesId, createdById: f.users.anna.id },
    { title: "Serie", kind: "rehearsal", date: isoTag(-7), seriesId, createdById: f.users.anna.id },
    { title: "Serie", kind: "rehearsal", date: isoTag(7), seriesId, createdById: f.users.anna.id },
    { title: "Serie", kind: "rehearsal", date: isoTag(14), seriesId, createdById: f.users.anna.id },
  ]);
});
...
describe("fetchSeriesInstances", () => {
  it("liefert alle Instanzen der Serie, nach Datum sortiert", async () => {
    const instances = await fetchSeriesInstances(seriesId);
  ...
  it("liefert eine leere Liste für eine unbekannte Serien-ID", async () => {
    const instances = await fetchSeriesInstances("gibt-es-nicht");
